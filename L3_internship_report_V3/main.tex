...
Even if the original theory had experimental failure, this Kaluza-Klein mechanism opened a whole new way of thinking about unification, 
and it is now at the heart of modern approaches like supergravity \cite{freedman2012supergravity} and string theory.\\
Another important part of physics since the begining as been the discovery of symetrie, indeed thanks to symetrie we can greatly reduce the level of difficulty of some theory, and
in other field we can even show that some multiple therory describe infact the same universe, but with a slightly diffenrent description, this has played a big part in string theory,
 and super gravity, or any advanced theory.\\
 In this internship we will begin by describing what is a field theory to seek how to describe the different action of the field we will be using, after that we will
  to follow the step of kaluza and klein by doing a dimensional reduction from D+E dimension to 4 dimension, However we will note juste use the classical Einstein action,
  but the NS-NS action of super gravity which basically add to new field, we will do this by using the vielbein formalism as it is useful for the descriptionof fermion even here we will solely try to describe bosonics fields,
...
\end{equation*}
\section{NS-NS Action Of supergravity}
In this section we perform a Kaluza-Klein reduction on the NS-NS action which is
...
We consider an universe in $D+E$ dimensions, with $D=4$, our goal will be to determine how an observer confined to the $D$-dimensional world perceives the extra dimensions.
...
\end{equation}
\subsection{Observation of gauge} 
...
\begin{definition}
    ...
    \end{equation*}
\end{definition}
Now we will consider that the action of the group on anything that does not have internal index is null so for the metric and $\phi$ we have
\begin{align}
    &g_{\mu \nu} \rightarrow g_{\mu \nu},\\
    &\phi \rightarrow \phi
\end{align}
...
&=\frac{1}{8}tr(\partial_\mu M^{-1}\partial^\mu M),
    \end{aligned}
\end{equation}
The O(E,E) then become manifest as M transformation under a global O(E,E)  transformation is
\begin{equation*}
