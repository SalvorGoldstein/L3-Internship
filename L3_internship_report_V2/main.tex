...
In general relativity we often encounter difficult to handle metrics such as the Kerr-Newman metric. A good idea we can have is to then work in an orthonormal basis $\{e^a_\mu \}$, which follow:
\begin{equation} \label{vieldef}
    g_{\mu \nu} V^{\mu}_{a}V^{\nu}_b = \eta_{ab}
\end{equation}
where g is the original metric and $\eta$ is the Minkowski metric.\\
We call this basis the vielbein. In this basis we will use the Minkowski metric to raise and lower indices.
Of course we can define the dual of the vielbein otherwise called the inverse vielbein with
\begin{equation} \label{inversviel}
    V^{a}_{\nu}V^{\nu}_b = \delta^a_b
\end{equation}
and of course that mean that we can go back with to the original metric with
\begin{equation} \label{toflatmet}
    g_{\mu \nu}  = \eta_{ab}V^{a}_{\mu}V^{b}_\nu
\end{equation}
Of course we can also change the basis any random vector, form or tensor with this, the method is to simply do
\begin{align} \label{changeofbasis}
    &T^a  =  V^a_\mu T^\mu \\
    &T_a =   V^\mu_a T_\mu  \\
    &T^a_b = V^a_\mu V^\nu_b T^\mu_\nu
\end{align}
Now in this internship we will not be in four dimensions, but rather in 4 time-space dimension, with E supplementarie space dimensions. And to pass from 4 + E total dimension to the other thing we use the vielbein in 4+E dimension the the 4 dimensions vielbein like this:
\begin{subequations} \label{lowdimv}
    ...
    \end{align}
\end{subequations}
Now of course other cange need to happen to the classical transformation for this to work, such as the classical christofells connexion which does not work anymore in this context
so we introduce the spin(or latin) connexion whith
\begin{equation} \label{spinconnexion}
    \nabla_\mu X^a_b = \partial_\mu X^a_b + \omega_\mu{}^a{}_c X^c_b - \omega_\mu{}^c{}_b X^a_c
\end{equation}
Of course we can still make link with the levi-civita conexion with the equations
\begin{align} \label{linkbetwinleviandlatin}
    &\Gamma_{\mu \lambda}^{\nu} = V^{\nu}_a \partial_\mu V^a_\lambda + V^\nu_aV^b_\lambda  \omega_\mu{}^a{}_b,\\
    &\omega_\mu{}^a{}_b = V^a_\nu V^\lambda_b \Gamma_{\mu \lambda}^{\nu} - V^\lambda_b \partial_\mu V^a_\lambda,
\end{align}
With those relation we can do prove the anti-symetrie of the last two index of the spin connexion
\begin{equation}
    \begin{aligned}
...
\begin{equation} \label{antisymlat}
    - \omega_{\mu a b }= \omega_{\mu b a}
\end{equation}
\subsection{Groupe and gaude transformation} \label{gaugeandgroupe}
Now we will look at both the gauge liberty of the theory and the symetrie of the O(E,E) group and it's action on different object.
so firstly the different gauge invarince we will encounter in this internship are $x'^{\mu} = x^\mu + \xi^\mu(x)$,\\ and $B_{\mu \nu} \rightarrow B_{\mu \nu} + \partial_mu \Lambda^{(2)}_\nu$
for the first one we can refer to the \cite{freedman2012supergravity} and use the Lie derivative to establish how different object will transorm:
...
\end{align}
Now we will look at the action of the O(E,E) group, but firstly, let's define what is the O(E,E) group:
\begin{definition} \label{O(E,E)def}
    ...
    \end{equation*}
\end{definition}
Now the question is how does this group act on a vector , a tensor a form or other things. We have
\begin{align} \label{O(E,E)action}
    &M^{\mu \nu} \rightarrow  M^{\mu \nu} + \Psi^\mu_\lambda M^{\lambda \gamma} \Psi^\nu_\gamma \\
    &A^\mu \rightarrow  A^\mu + \Psi^\mu_\nu A^\nu
\end{align}
...
\begin{subequations}
    \begin{align}
        &S_{\hat g} = \int d x^D\sqrt{- \hat g } e^{-\hat \phi} \hat R \label{Sg}\\ 
        &S_{\hat \phi} =  \int d x^D \sqrt{- \hat g } e^{-\hat \phi} \hat g^{\hat \mu \hat \nu}\partial_{\hat \mu}\hat \phi \partial^{\hat \nu}\hat \phi \label{Sphi}\\ 
...
\end{equation}
for the metric\\
Now that we have made this anstaz an intersting things to do is to verify what does object are by using the classical transformation $x'^{ \hat \mu} = x^{ \hat \mu} +\xi^{\mu}(x)$ and then using what we saw in  part \ref{gaugeandgroupe} that with the Lie derivate we can rewrote this transformation as being
Using the fact that the theory is invariant under the classical transformation $x'^\mu = x^\mu + \xi^\mu(x)$ \cite{scherk1979get}, which can be rewritten with \eqref{xitransfo} as 
 \begin{equation}
 ...
 \end{equation}
 we now apply this transformation to each block of the vielbein, in order to see whether it corresponds to some familiar type of field still thanks to \eqref{xitransfo} . Let us start with the block $\hat V^r_\mu$:
\begin{align*}
   ...
   &=  \xi^{\hat \rho}  \partial_{\hat \rho}( V^{ r}_{ \mu} +  \partial_{ \mu} \xi^{ \nu} V^{  r}_{\nu} \\
   &= (\xi^{\hat \rho}  \partial_{\hat \rho}V^{ r}_{ \mu} +  \partial_{ \mu} \xi^{ \nu} V^{  r}_{\nu} ) + \xi^{\hat \rho}  \partial_{\hat \rho} V^{ r}_{ \mu}
\end{align*}
We can also redo this computation using a different method:
\begin{align*}
    \delta \hat V^r_\mu &= \delta(  V^r_\mu)\\
    & =    \delta (V^r_\mu)
\end{align*}
...
\end{equation}
Te absence if hat is due to what we so in equation \eqref{lowdimv} \\
...
Now as in part \ref{part:vielbeinanstaz} we are going to observe the gauge rule to be sure that every object transorm as it should
...
and following the \cite{maharana1993noncompact} we observe an O(E,E) symetrie (defined here \ref{def:O(E,E)}.\\
