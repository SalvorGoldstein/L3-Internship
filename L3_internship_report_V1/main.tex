...
\begin{equation}
    \begin{aligned}
        S &= \frac{-1}{4\kappa^2}\int d x^D \int \frac{d y ^E}{\rho(E)} \hat V \hat V^{\hat \mu \hat r}\hat V^{\hat \nu \hat s} \hat R_{\hat \mu \hat \nu \hat r \hat s}\\
        &= \frac{-1}{4\kappa^2}\int d x^D \int \frac{d y ^E}{\rho(E)} \hat V \hat V^{\hat \mu \hat r}\hat V^{\hat \nu \hat s}(\partial_{\hat \nu} \hat \omega_{\hat \nu \hat r \hat s} + \hat \omega_{\hat \mu \hat t \hat s} \hat \omega_{\hat \nu \hat t \hat s}-\partial_{\hat \nu} \hat \omega_{\hat \nu \hat r \hat s} - \hat \omega_{\hat \mu \hat t \hat s} \hat \omega_{\hat \nu \hat t \hat s})\\
        &=\frac{-1}{4\kappa^2}\int d x^D \int \frac{d y ^E}{\rho(E)} \hat V \big(\hat \omega^{\hat r}{}_{\hat s \hat t} \hat \omega^{\hat s \hat t}{}_{\hat r} + \hat \omega^{\hat r}{}_{ \hat r \hat t}\hat \omega^{\hat s }{}_{\hat s}{}^{\hat t}\big)\\
    \end{aligned}
    \end{equation}
